...
const fetchAndSaveAllPlayerStatistics = async (playerId) => {
    try {
      ...
      return { message: 'Player statistics for all seasons saved/updated successfully' };
  
    } catch (error) {
      throw new CustomError(error.message || 'Error processing player statistics', error.statusCode || 500);
    }
  };
...
const fetchPlayerStatisticsByPlayerId = async (playerId, season) => {
  ...
  try {
    ...
    if (!data || !data.response) {
      throw new CustomError(`Player statistics not found for playerId ${playerId} season ${season}`, 404);
    }
    ...
    throw new CustomError(error.message || 'Error fetching player statistics', error.response ? error.response.status : 500);
  }
};
...
const savePlayerStatistics = async (playerId, season, statistics) => {
    try {
      ...
      throw new Error(`Error saving or updating player statistics: ${error.message}`); // Propagate error to be handled in the controller
    }
  };
