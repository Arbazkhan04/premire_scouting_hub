...
const queue = async.queue(async (task, callback) => {
  ...
  for (let attempt = 0; attempt <= 3; attempt++) {
    ...
    try {
      const response = await axios.get(task.url, {
        params: task.params,
        headers: task.headers, // Dynamically use the passed headers
      });
      ...
      if (error.response && error.response.status === 429) {
        ...
        callback(error); // Return error if it's not a rate limit error
        return;
      }
    }
  }
 ...
 * @param {string} url - The API endpoint.
 * @param {object} params - Query parameters.
 * @param {object} headers - Custom headers (API keys, hosts, etc.).
 * @returns {Promise<object>} - API response data.
 */
const apiRequest = (url, params = {}, headers = {}) => {
  return new Promise((resolve, reject) => {
    // Push task into the queue
    queue.push({ url, params, headers }, (err, result) => {
      if (err) {
        reject(err);
      } else {
        resolve(result);
